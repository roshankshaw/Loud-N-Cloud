function updateCases(districts, data) {
  ...
  for (let i = 0; i < district.length; i++) {
    ...
    if (data[districtId] !== undefined) {
      ...
      literacy.Literacy = new Array(
        data[districtId].confirmed / maxConfirmed,
        (totalVaccines / 1000000).toFixed(2),
        data[districtId].confirmed
      );

      //  color will be decided by normalised values of active covid
      // MaleLiteracy decides color
      literacy.MaleLiteracy = new Array(
        data[districtId].active / maxActive,
        data[districtId].active
      );
      literacy.FemaleLiteracy = new Array(
        data[districtId].recovered / maxRecovered,
        data[districtId].recovered
      );
